...
import com.github.mikephil.charting.Logs;
...
public class KLineDataManage {
        ...
        if (object != null) {
            ...
            if (data != null) {
                ...
                for (int i = 0; i < data.length(); i++) {
                    ...
                    Logs.s("  k线图数据0   " + data.optJSONArray(i).optLong(0, 0L));
                    Logs.s("  k线图数据1   " + data.optJSONArray(i).optDouble(1));
                    Logs.s("  k线图数据2   " + data.optJSONArray(i).optDouble(2));
                    Logs.s(" k线图数据3    " + data.optJSONArray(i).optDouble(3));
                    Logs.s(" k线图数据4    " + data.optJSONArray(i).optDouble(4));
                    Logs.s(" k线图数据5    " + NumberUtils.stringNoE10ForVol(Double.isNaN(data.optJSONArray(i).optDouble(5)) ? 0 : data.optJSONArray(i).optDouble(5)));
                    Logs.s("  k线图数据6   " + NumberUtils.stringNoE10ForVol(Double.isNaN(data.optJSONArray(i).optDouble(6)) ? 0 : data.optJSONArray(i).optDouble(6)));
                    Logs.s(" k线图数据7    " + data.optJSONArray(i).optDouble(7));
                    Logs.s("  k线图数据8   " + data.optJSONArray(i).optDouble(8));
                    Logs.s(" k线图数据9    " + data.optJSONArray(i).optDouble(9));
                    Logs.s("  k线图数据10   " + data.optJSONArray(i).optDouble(10));
                    Logs.s(" k线图数据11    " + data.optJSONArray(i).optDouble(11));
                    Logs.s("  k线图数据12   " + data.optJSONArray(i).optDouble(12));
    ...
    public String toString() {
        return "KLineDataManage{" +
                ...
                ", bollCandleDataSet=" + bollCandleDataSet +
                ", lineDataMA=" + lineDataMA +
                ", lineDataMACD=" + lineDataMACD +
                ", macdData=" + macdData +
                ", deaData=" + deaData +
                ", difData=" + difData +
                ", lineDataKDJ=" + lineDataKDJ +
                ", kData=" + kData +
                ", dData=" + dData +
                ", jData=" + jData +
                ", lineDataBOLL=" + lineDataBOLL +
                ", bollDataUP=" + bollDataUP +
                ", bollDataMB=" + bollDataMB +
                ", bollDataDN=" + bollDataDN +
                ", lineDataRSI=" + lineDataRSI +
                ", rsiData6=" + rsiData6 +
                ", rsiData12=" + rsiData12 +
                ", rsiData24=" + rsiData24 +
